...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        self.weights_hidden_to_output = np.random.normal(0.0, self.hidden_nodes**-0.5, size=(self.hidden_nodes, self.output_nodes))
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            delta_weights_i_h += (self.lr * hidden_error_term * hidden_inputs[:,None].T).T
            # Weight step (hidden to output)
            delta_weights_h_o += (self.lr * output_error_term * final_inputs[:,None].T).T
...
# ## Unit tests
# 
# Run these unit tests to check the correctness of your network implementation. This will help you be sure your network
# was implemented correctly befor you starting trying to train it. These tests must all be successful to pass the project.
#
# In[47]:
...
for ii in range(iterations):
    ...
    sys.stdout.write("\rProgress: {:2.1f}".format(100 * ii/float(iterations)) + "% ... Training loss: " +
                     str(train_loss)[:5]  + " ... Validation loss: " + str(val_loss)[:5])
    sys.stdout.flush()
